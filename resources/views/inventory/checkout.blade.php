            ...
            <div class="box box-default">
                <form class="form-horizontal" method="post" action="" autocomplete="off">
                    <div class="box-header with-border">
                        <h2 class="box-title"> {{ trans('admin/hardware/form.tag') }} {{ $inventory->inventory_tag }}</h2>
                    </div>
                    <div class="box-body">
                        {{csrf_field()}}
                        @if ($inventory->company)
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <p class="form-control-static">{!! $inventory->company->present()->formattedNameLink  !!}</p>
                                </div>
                            </div>
                        @endif


                        @if ($inventory->model->category)
                            <!-- category name -->
                            <div class="form-group">
                                <label class="col-sm-3 control-label">{{ trans('general.category') }}</label>
                                <div class="col-md-6">
                                    <p class="form-control-static">{!! $inventory->model->category->present()->formattedNameLink  !!}</p>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                <p class="form-control-static" style="padding-top: 7px;">
                                    @if (($inventory->model) && ($inventory->model->name))
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'error' : '' }}">
                            ...
                            <div class="col-md-8">
                                <input class="form-control" type="text" name="name" id="name"
                                       value="{{ old('name', $inventory->name) }}" tabindex="1">
                        ...
                        <!-- We have to pass unselect here so that we don't default to the inventory that's being checked out. We want that inventory to be pre-selected everywhere else. -->
                        @include ('partials.forms.edit.asset-select', ['translated_name' => trans('general.select_asset'), 'fieldname' => 'assigned_asset', 'company_id' => $inventory->company_id, 'unselect' => 'true', 'style' => session('checkout_to_type') == 'asset' ? '' : 'display: none;'])
                        ...
                        <div class="form-group {{ $errors->has('note') ? 'error' : '' }}">
                            ...
                            <div class="col-md-8">
                                ...
                                name="note">{{ old('note', $inventory->note) }}</textarea>
                        ...
                        @if ($inventory->requireAcceptance() || $inventory->getEula() || ($snipeSettings->webhook_endpoint!=''))
                            <div class="row">
                            <div class="notification-callout">
                                <div class="col-md-8 col-md-offset-3">
                                    <div class="callout callout-info">

                                        @if ($inventory->requireAcceptance())
                                            <x-icon type="email" />
                                            {{ trans('admin/categories/general.required_acceptance') }}
                                            <br>
                                        @endif

                                        @if ($inventory->getEula())
                                            <x-icon type="email" />
                    ...
                    <x-redirect_submit_options
                            ...
                            :disabled_select="!$inventory->model"
                            :options="[
                                'index' => trans('admin/hardware/form.redirect_to_all', ['type' => trans('general.inventories')]),
                                'item' => trans('admin/hardware/form.redirect_to_type', ['type' => trans('general.inventory')]),
