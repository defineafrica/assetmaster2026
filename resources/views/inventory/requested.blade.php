                ...
                <div class="row"><!-- .row -->
                    <div class="col-md-12"><!-- col-md-12 -->
                        ...
                        <table
                            ...
                            data-export-options='{
                            "fileName": "export-inventoryrequests-{{ date('Y-m-d') }}",
                            ...
                            <tbody>
                                @foreach ($requestedItems as $request)
                                    ...
                                    {{ csrf_field() }}
                                        <td>
                                        @if (($request->itemType() == "inventory") && ($request->requestable))
                                        ...
                                        <td>
                                            ...
                                            @if ($request->itemType() == "inventory")
                                                <a href="{{ config('app.url') }}/inventories/{{ $request->requestable->id }}">
                                                    {{ $request->name() }}
                                                </a>
                                        ...
                                        <td>
                                            @if ($request->itemType() == "inventory")
                                                @if ($request->requestable->assigned_to=='')
                                                    <a href="{{ config('app.url') }}/inventories/{{ $request->requestable->id }}/checkout" class="btn btn-sm bg-maroon" data-tooltip="true" title="{{ trans('general.checkout_user_tooltip') }}">{{ trans('general.checkout') }}</a>
                                                @else
                                                    <a href="{{ config('app.url') }}/inventories/{{ $request->requestable->id }}/checkin" class="btn btn-sm bg-purple" data-tooltip="true" title="{{ trans('general.checkin_tooltip') }}">{{ trans('general.checkin') }}</a>
                                                @endif
                                            @endif
                                        </td>
