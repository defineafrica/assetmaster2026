...
<center><img src="{{ $item->getImageUrl() }}" alt="Inventory" style="max-width: 570px;"></center>
@endif
...
| **{{ trans('mail.inventory_name') }}** | {{ $item->name }} |
@endif
@if (($item->name!=$item->inventory_tag))
| **{{ trans('mail.inventory_tag') }}** | {{ $item->inventory_tag }} |
@endif
...
@endif
@if (isset($item->model))
| **{{ trans('general.inventory_model') }}** | {{ $item->model->name }} |
@endif
