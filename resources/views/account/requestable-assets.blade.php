...
<div class="row">
    <div class="col-md-12">
        ...
        @if (($assets->count() < 1) && ($models->count() < 1) && ($inventories->count() < 1))
        ...
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
                ...
                <li class="{{ ($assets->count() == 0) ? 'active' : '' }}">
                    <a href="#inventories" data-toggle="tab" title="{{ trans('general.inventories') }}">{{ trans('general.inventories') }}
                        <span class="badge badge-secondary"> {{ $inventories->count()}}</span>
                    </a>
                </li>
                @endif
                @if ($models->count() > 0)
            ...
            <div class="tab-content">
                ...
                @if ($inventories->count() > 0)
                <div class="tab-pane fade in {{ ($assets->count() == 0) ? 'active' : '' }}" id="inventories">
                    <div class="row">
                        <div class="col-md-12">
                            <table
                                data-cookie-id-table="requestableInventoriesListingTable"
                                data-id-table="requestableInventoriesListingTable"
                                data-side-pagination="server"
                                data-show-export="false"
                                data-show-footer="false"
                                data-sort-order="asc"
                                data-sort-name="name"
                                data-toolbar="#inventoriesBulkEditToolbar"
                                data-bulk-button-id="#bulkInventoryEditButton"
                                data-bulk-form-id="#inventoriesBulkForm"
                                id="inventoriesListingTable"
                                class="table table-striped snipe-table"
                                data-url="{{ route('api.inventories.requestable', ['requestable' => true]) }}">

                                <thead>
                                    <tr>
                                        <th class="col-md-1" data-field="image" data-formatter="imageFormatter" data-sortable="true">{{ trans('general.image') }}</th>
                                        <th class="col-md-2" data-field="inventory_tag" data-sortable="true" >{{ trans('general.inventory_tag') }}</th>
                                        <th class="col-md-2" data-field="model" data-sortable="true">{{ trans('admin/hardware/table.asset_model') }}</th>
                                        <th class="col-md-2" data-field="model_number" data-sortable="true">{{ trans('admin/models/table.modelnumber') }}</th>
                                        <th class="col-md-2" data-field="name" data-sortable="true">{{ trans('admin/hardware/form.name') }}</th>
                                        <th class="col-md-3" data-field="serial" data-sortable="true">{{ trans('admin/hardware/table.serial') }}</th>
                                        ...
                                        @foreach(\App\Models\CustomField::get() as $field)
                                            ...
                                            @endif
                                        @endforeach
                                        <th class="col-md-1" data-formatter="inventoryRequestActionsFormatter" data-field="actions" data-sortable="false">{{ trans('table.actions') }}</th>
                                    </tr>
                                </thead>
                            </table>
                        </div>
                ...
                @if ($models->count() > 0)
                <div class="tab-pane fade in {{ ($assets->count() == 0 && $inventories->count() == 0) ? 'active' : '' }}" id="models">
                    <div class="row">
                        <div class="col-md-12">
                                <table
