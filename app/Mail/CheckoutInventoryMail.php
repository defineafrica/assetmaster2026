...
class CheckoutInventoryMail extends BaseMailable
{
     ...
     * @throws \Exception
     */
    public function __construct(Inventory $inventory, $checkedOutTo, ?User $checkedOutBy, $acceptance, $note, bool $firstTimeSending = true)
    {
        $this->item = $inventory;
    ...
    public function content(): Content
    {
        ...
        else if($this->target instanceof Inventory){
        ...
        return new Content(
            markdown: 'mail.markdown.checkout-inventory',
            with:  [
                'item'          => $this->item,
                'admin'         => $this->admin,
    ...
    private function getSubject(): string
    {
        if ($this->firstTimeSending) {
            return trans('mail.Inventory_Checkout_Notification', ['tag' => $this->item->inventory_tag]);
        }

        return trans('mail.unaccepted_inventory_reminder');
    }
